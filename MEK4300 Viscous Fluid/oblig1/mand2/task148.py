...
def newton():
	...
	viz = plot(f_)
	viz.write_png('148newton1')
	viz = plot(h_)
	viz.write_png('148newton2')
	interactive()
...
def picard():
	...
	F = h*vf*dx - f.dx(0)*vf*dx - inner(grad(h),grad(vh))*dx + 2*f_*h.dx(0)*vh*dx + 		vh*dx - h*h_*vh*dx
	...
	viz = plot(f_)
	viz.write_png('148picard1')
	viz = plot(h_)
	viz.write_png('148picard2')
	interactive()
