...
class Poission():
    ...
    def supg(self, my, u_, u_e):
        v = u_.vector() - u_e.vector()
        gamma = 0.5*div(u_)
        test = norm(v, 'L2')
        supg = my*errornorm(u_e, u_, norm_type='l2', degree_rise=3) #+ norm(np.sqrt(gamma)*v)
        return supg
    ...
    def calc(self, i, my, output, upwind, imp_norm):
        ...
        u_enorm = interpolate(Expression('1./(exp(1./my)- 1 ) * (exp(x[0]/my) - 1)', my = my), V)
        f = Constant(0)
        if upwind == True:
            beta_val = 0.5
        ...
        if my == 1:
            ...
            if imp_norm == "supg":
                supg = self.supg(my, u_, u_enorm)
                print "SUPG = %.4f" % supg
                self.y[self.count] = np.log(supg)
...
for j in [1, 2]:
    ...
    for i in h:
        for m in my:
            prob.set_mesh(i)
            prob.calc(j, m, output = False, upwind = True, imp_norm = "supg") #improvised norm
    print '###########################################################\n'
